...
@torch.no_grad()
def flow_matching_multi_inference_compose_v2(models, normed_ima_feats,steps, distangle=False, manifold=None):
    ...
    stepsize = [1, 0.5]
    if steps == 0:
        return normed_ima_feats[0]
    else:
        
        ts =  torch.linspace(0,1,steps+1).unsqueeze(1).to(device)
        current_points_1 = normed_ima_feats[0].float()
        
        current_points_attr = normed_ima_feats[0].float()
        current_points_obj = normed_ima_feats[0].float()
        for i in range(len(ts)-1):
            drift_attr = models[1](current_points_attr, ts[i])
            current_points_attr = current_points_attr +drift_attr*(ts[i+1]-ts[i])*1
            drift_obj = models[2](current_points_obj, ts[i])
            current_points_obj = current_points_obj +drift_obj*(ts[i+1]-ts[i])*1
        
            drift = drift_attr + drift_obj
            current_points_1 = current_points_1 +drift*(ts[i+1]-ts[i])*1
        transfer_features = current_points_1
        transfer_features = transfer_features / transfer_features.norm(dim=-1, keepdim=True)
        return transfer_features

@torch.no_grad()
def flow_matching_multi_inference(model, normed_ima_feat,steps, distangle=False, manifold=None):
    ...
    stepsize = [1, 0.5]
    if steps == 0:
        return normed_ima_feat
    else:
        
        ts =  torch.linspace(0,1,steps+1).unsqueeze(1).to(device)
        current_points_1 = normed_ima_feat.float()
        for i in range(len(ts)-1):
            if distangle:
                _,_, drift = model(current_points_1, ts[i],train=False)
                current_points_1 = current_points_1 +drift*(ts[i+1]-ts[i])*1
            else:
                drift = model(current_points_1, ts[i])
                current_points_1 = current_points_1 +drift*(ts[i+1]-ts[i])*1
        transfer_features = current_points_1
        transfer_features = transfer_features / transfer_features.norm(dim=-1, keepdim=True)
        return transfer_features
...
def predict_logits_fm_joint_troika_v2_3path(model, cfm_models, dataset, steps,path_2,compose=False, weight_1=0.5,device=None):
    ...
    with torch.no_grad():
        for idx, data in tqdm(
            ...
            data = tuple(to_dev(x) for x in data)
            
            text_embeddings,normalized_img_features = model(data,pairs)
            pair_text_embeddings, attr_text_embeddings, obj_text_embeddings = text_embeddings
            
            transfered_composition_features = []
            transfered_attribute_features = []
            transfered_object_features = []
            
            if path_2:
                if compose:
                    transfer_features = flow_matching_multi_inference_compose_v2(cfm_models, normalized_img_features, steps)
                    transfered_composition_features.append(transfer_features)
                else:
                    transfer_features = normalized_img_features[0]
                    transfered_composition_features.append(transfer_features)
                    
                for j in range(1,3):
                    transfer_features = flow_matching_multi_inference(
                        cfm_models[j], normalized_img_features[0], steps, distangle=False
                    )
                    transfered_composition_features.append(transfer_features)
            else:
                transfer_features = flow_matching_multi_inference(
                                cfm_models[0], normalized_img_features[0], steps, distangle=False
                            )
            
            
            for j in range(1,3):
                transfer_features_attr = flow_matching_multi_inference(
                            cfm_models[j], normalized_img_features[1], steps, distangle=False
                        )
                transfered_attribute_features.append(transfer_features_attr)
                
            for j in range(1,3):
                transfer_features_obj = flow_matching_multi_inference(
                            cfm_models[j], normalized_img_features[2], steps, distangle=False
                        )
                transfered_object_features.append(transfer_features_obj)
          
            # logits = transfer_features @ pair_text_embeddings.t() * model.clip.logit_scale.exp()
            logits = torch.einsum(
                    "bd, bkd->bk", 
                    transfered_composition_features[0], 
                    pair_text_embeddings * model.clip.logit_scale.exp())
            logits_attr = torch.einsum(
                    "bd, bkd->bk", 
                    transfered_attribute_features[0], 
                    attr_text_embeddings * model.clip.logit_scale.exp())
            logits_attr_obj = torch.einsum(
                    "bd, bkd->bk", 
                    transfered_object_features[0], 
                    attr_text_embeddings * model.clip.logit_scale.exp())
            logits_attr_comp = torch.einsum(
                    "bd, bkd->bk", 
                    transfered_composition_features[1], 
                    attr_text_embeddings * model.clip.logit_scale.exp())
            
            
            logits_obj = torch.einsum(
                    "bd, bkd->bk", 
                    transfered_object_features[1], 
                    obj_text_embeddings * model.clip.logit_scale.exp())
            logits_obj_attr = torch.einsum(
                    "bd, bkd->bk", 
                    transfered_composition_features[2], 
                    obj_text_embeddings * model.clip.logit_scale.exp())
            logits_obj_comp = torch.einsum(
                    "bd, bkd->bk", 
                    transfered_attribute_features[1], 
                    obj_text_embeddings * model.clip.logit_scale.exp())
           
            
            
            comp_logits = copy.deepcopy(logits)
            attr_pred = F.softmax(logits_attr, dim=-1)
            obj_pred = F.softmax(logits_obj, dim=-1)
            
            attr_pred_obj = F.softmax(logits_attr_obj, dim=-1)
            attr_pred_comp = F.softmax(logits_attr_comp, dim=-1)
            obj_pred_attr = F.softmax(logits_obj_attr, dim=-1)
            obj_pred_comp = F.softmax(logits_obj_comp, dim=-1)
            
            for i_comp in range(logits.shape[-1]):
                weighted_attr_pred = attr_pred[:, pairs[i_comp][0]] 
                weighted_attr_pred_obj = attr_pred_obj[:, pairs[i_comp][0]] 
                weighted_attr_pred_comp = attr_pred_comp[:, pairs[i_comp][0]] 
                weighted_obj_pred = obj_pred[:, pairs[i_comp][1]] 
                weighted_obj_pred_attr = obj_pred_attr[:, pairs[i_comp][1]] 
                weighted_obj_pred_comp = obj_pred_comp[:, pairs[i_comp][1]] 
                
                comp_logits[:, i_comp] = comp_logits[:, i_comp]  + weighted_attr_pred * weighted_obj_pred + weight_1*weighted_attr_pred_obj*weighted_obj_pred_attr + weight_1*weighted_attr_pred_comp*weighted_obj_pred_comp
            ...
            composed_logits = torch.cat([composed_logits, comp_logits.cpu()], dim=0)
    ...
    return all_logits, all_attr_gt, all_obj_gt, all_pair_gt, composed_logits




def predict_logits_fm_joint_troika_v2_3path_wGate_v2(model, cfm_models,gater, dataset, steps,path_2,compose=False, weight_1=0.5,end_points=1.0, normalize=False):
    """Function to predict the cosine similarities between the
    images and the attribute-object representations. The function
    also returns the ground truth for attributes, objects, and pair
    of attribute-objects.
    ...
    with torch.no_grad():
        for idx, data in tqdm(
            ...
            if path_2:
                if compose:
                    transfer_features = flow_matching_multi_inference_compose_v2_wGate_v2(cfm_models, gater, normalized_img_features, steps, end_points=end_points, normalize=normalize)
                    transfered_composition_features.append(transfer_features)
                else:
                    transfer_features = normalized_img_features[0]
                    transfered_composition_features.append(transfer_features)
...
@torch.no_grad()
def flow_matching_multi_inference_compose_v2_wGate_v2(models, gater,normed_ima_feats,steps, distangle=False, manifold=None, end_points=1.0, normalize=False):
    ...
    total = 0
    stepsize = [1, 0.5]
    if steps == 0 or end_points==0:
        return normed_ima_feats[0]
    else:
        
        ts =  torch.linspace(0,end_points,steps+1).unsqueeze(1).to(device)
        current_points_1 = normed_ima_feats[0].float()
        ...
        for i in range(len(ts)-1):
            ...
            if normalize:
                drift_attr = drift_attr / drift_attr.norm(dim=-1, keepdim=True)
                drift_obj = drift_obj / drift_obj.norm(dim=-1, keepdim=True)
         
            a, b, gamma, drift = gater(drift_attr, drift_obj, tau=1) 
            current_points_1 = current_points_1 +drift*(ts[i+1]-ts[i])*1
        transfer_features = current_points_1
        transfer_features = transfer_features / transfer_features.norm(dim=-1, keepdim=True)
        return transfer_features
