...
class Troika(nn.Module):
    ...
    def forward(self, batch, idx):
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            text_feat.append(cmt_text_features)
            # logits.append(
            #     torch.einsum(
            #         "bd, bkd->bk", 
            #         normalized_img_features[i_element], 
        ...
        return text_feat, normalized_img_features
    
    def composition_analysis(self, batch, idx):
        batch_img = batch[0].cuda()
        # print("batch_img shape",batch_img.shape)
        b = batch_img.shape[0]
        images,att_label,obj_label ,pair = batch
        images, att_label, obj_label, pair  = images.cuda(), att_label.cuda(), obj_label.cuda(), pair.cuda()
        l, _ = idx.shape
        batch_img, batch_patch = self.encode_image(batch_img.type(self.clip.dtype))
        batch_img_features = [batch_img, self.attr_disentangler(batch_img), self.obj_disentangler(batch_img)]
        normalized_img_features = [feats / feats.norm(dim=-1, keepdim=True) for feats in batch_img_features]
        ...
        img_comp_diff_list = list()
        text_comp_diff_list = list()
        img_text_comp_diff_list = list()
        labels_list = [pair, att_label, obj_label]
        # norm_pair_text_features = text_feat[0][torch.arange(16), pair, :]   .to(device)
        # norm_attr_text_features = text_feat[1][torch.arange(16), att_label, :].to(device)
        # norm_obj_text_features = text_feat[2][torch.arange(16), obj_label, :].to(device)
        # img_comp_diff_batch = normalized_img_features[1] + normalized_img_features[2] - normalized_img_features[0]
        img_comp_diff_batch = torch.norm(normalized_img_features[1] + normalized_img_features[2] - normalized_img_features[0], dim=1)
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            cmt_text_features_gt = cmt_text_features[torch.arange(b), labels_list[i_element], :]
            normalized_text_features.append(cmt_text_features_gt)
        text_comp_diff_batch = torch.norm(normalized_text_features[1] + normalized_text_features[2] - normalized_text_features[0], dim=1)
        img_text_comp_diff_batch = normalized_text_features[1] + normalized_text_features[2] - normalized_text_features[0] - (normalized_img_features[1] + normalized_img_features[2] - normalized_img_features[0])
        
        img_text_comp_diff_batch = torch.norm(normalized_text_features[1] + normalized_text_features[2] - normalized_text_features[0] - (normalized_img_features[1] + normalized_img_features[2] - normalized_img_features[0]), dim=1)
        return img_comp_diff_batch, text_comp_diff_batch, img_text_comp_diff_batch
    
    def feat_analysis(self, batch, idx):
        batch_img = batch[0].cuda()
        # print("batch_img shape",batch_img.shape)
        b = batch_img.shape[0]
        images,att_label,obj_label ,pair = batch
        ...
        # norm_obj_text_features = text_feat[2][torch.arange(16), obj_label, :].to(device)
        # img_comp_diff_batch = normalized_img_features[1] + normalized_img_features[2] - normalized_img_features[0]
        # img_comp_diff_batch_1 = torch.norm(normalized_img_features[0]  - normalized_img_features[1], dim=1)
        # img_comp_diff_batch_2 = torch.norm(normalized_img_features[0]  - normalized_img_features[2], dim=1)
        # img_comp_diff_batch_3 = torch.norm(normalized_img_features[1]  - normalized_img_features[2], dim=1)
        img_comp_diff_batch_1 = 1- F.cosine_similarity(
            normalized_img_features[0], 
            normalized_img_features[1], 
            dim=1
        )
        img_comp_diff_batch_2 = 1- F.cosine_similarity(
            normalized_img_features[0], 
            normalized_img_features[2], 
            dim=1
        )
        img_comp_diff_batch_3 = 1- F.cosine_similarity(
            normalized_img_features[1], 
            normalized_img_features[2], 
            dim=1)
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            cmt_text_features_gt = cmt_text_features[torch.arange(b), labels_list[i_element], :]
            normalized_text_features.append(cmt_text_features_gt)
        # text_comp_diff_batch_1 = torch.norm(normalized_text_features[0]  - normalized_text_features[1], dim=1)
        
        # text_comp_diff_batch_2 = torch.norm(normalized_text_features[0]  - normalized_text_features[2], dim=1)
        # text_comp_diff_batch_3 = torch.norm(normalized_text_features[1]  - normalized_text_features[2], dim=1)
        text_comp_diff_batch_1 = 1- F.cosine_similarity(
            normalized_text_features[0], 
            normalized_text_features[1], 
            dim=1
        )
        text_comp_diff_batch_2 = 1- F.cosine_similarity(
            normalized_text_features[0], 
            normalized_text_features[2], 
            dim=1
        )
        text_comp_diff_batch_3 = 1- F.cosine_similarity(
            normalized_text_features[1], 
            normalized_text_features[2], 
            dim=1)
        # img_text_comp_diff_batch = normalized_text_features[1] + normalized_text_features[2] - normalized_text_features[0] - (normalized_img_features[1] + normalized_img_features[2] - normalized_img_features[0])
        ...
        # img_text_comp_diff_batch = torch.norm(normalized_text_features[1] + normalized_text_features[2] - normalized_text_features[0] - (normalized_img_features[1] + normalized_img_features[2] - normalized_img_features[0]), dim=1)
        return img_comp_diff_batch_1,img_comp_diff_batch_2, img_comp_diff_batch_3, text_comp_diff_batch_1, text_comp_diff_batch_2, text_comp_diff_batch_3